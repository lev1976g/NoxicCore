...
{
	auctionLock.AcquireReadLock();
	HM_NAMESPACE::hash_map<uint32, Auction*>::iterator itr = auctions.find(Id);
...
void WorldSession::HandleCancelAuction(WorldPacket & recv_data)
{
	...
	{
		Log.Error("WordSession", "Cannot find auctionhouse for creature %u.", guid);
		return;
	}
	...
	if(!auct->pItem)
	{
		Log.Error("WordSession", "Auction id %u has non-existent item (guid: %u).", auction_id, auct->pItem->GetLowGUID());
		return;
	}
