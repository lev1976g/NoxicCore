...
uint32 SpellProc::CalcProcChance(Unit* victim, SpellEntry* CastingSpell)
{
	...
	if(mTarget->IsPlayer() && mOrigSpell && mOrigSpell->AttributesEx & ATTRIBUTESEX_REQ_COMBO_POINTS1 && mOrigSpell->AttributesExD & ATTRIBUTESEXD_PROC_ONLY_ON_CASTER)
...
{
	if(mOrigSpell != NULL && mOrigSpell->AttributesExC & ATTRIBUTESEXC_CAN_PROC_WITH_TRIGGERED)
		return true;
	return false;
}
