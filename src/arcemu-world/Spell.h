...
// *******************************************
// Spell Attributes definitions (TrinityCore)
// *******************************************

enum Attributes
{
    ATTRIBUTES_NULL									= 0x00000000, //  2^x
    ATTRIBUTES_UNK0									= 0x00000001, //  0
    ATTRIBUTES_REQ_AMMO								= 0x00000002, //  1 on next ranged
    ATTRIBUTES_ON_NEXT_SWING						= 0x00000004, //  2
    ATTRIBUTES_IS_REPLENISHMENT						= 0x00000008, //  3 not set in 3.0.3
    ATTRIBUTES_ABILITY								= 0x00000010, //  4 client puts 'ability' instead of 'spell' in game strings for these spells
    ATTRIBUTES_TRADESPELL							= 0x00000020, //  5 trade spells (recipes), will be added by client to a sublist of profession spell
    ATTRIBUTES_PASSIVE								= 0x00000040, //  6 Passive spell
    ATTRIBUTES_HIDDEN_CLIENTSIDE					= 0x00000080, //  7 Spells with this attribute are not visible in spellbook or aura bar
    ATTRIBUTES_HIDE_IN_COMBAT_LOG					= 0x00000100, //  8 This attribute controls whether spell appears in combat logs
    ATTRIBUTES_TARGET_MAINHAND_ITEM					= 0x00000200, //  9 Client automatically selects item from mainhand slot as a cast target
    ATTRIBUTES_ON_NEXT_SWING_2						= 0x00000400, // 10
    ...
    ATTRIBUTES_OUTDOORS_ONLY						= 0x00008000, // 15 Only useable outdoors.
    ATTRIBUTES_NOT_SHAPESHIFT						= 0x00010000, // 16 Not while shapeshifted
    ATTRIBUTES_ONLY_STEALTHED						= 0x00020000, // 17 Must be in stealth
    ATTRIBUTES_DONT_AFFECT_SHEATH_STATE				= 0x00040000, // 18 client won't hide unit weapons in sheath on cast/channel
    ATTRIBUTES_LEVEL_DAMAGE_CALCULATION				= 0x00080000, // 19 spelldamage depends on caster level
    ATTRIBUTES_STOP_ATTACK_TARGET					= 0x00100000, // 20 Stop attack after use this spell (and not begin attack if use)
    ATTRIBUTES_IMPOSSIBLE_DODGE_PARRY_BLOCK			= 0x00200000, // 21 Cannot be dodged/parried/blocked
    ATTRIBUTES_CAST_TRACK_TARGET					= 0x00400000, // 22 Client automatically forces player to face target when casting
    ATTRIBUTES_CASTABLE_WHILE_DEAD					= 0x00800000, // 23 castable while dead?
    ATTRIBUTES_CASTABLE_WHILE_MOUNTED				= 0x01000000, // 24 castable while mounted
    ATTRIBUTES_DISABLED_WHILE_ACTIVE				= 0x02000000, // 25 Activate and start cooldown after aura fade or remove summoned creature or go
    ATTRIBUTES_NEGATIVE_1							= 0x04000000, // 26 Many negative spells have this attr
    ATTRIBUTES_CASTABLE_WHILE_SITTING				= 0x08000000, // 27 castable while sitting
    ATTRIBUTES_CANT_USED_IN_COMBAT					= 0x10000000, // 28 Cannot be used in combat
    ATTRIBUTES_UNAFFECTED_BY_INVULNERABILITY		= 0x20000000, // 29 unaffected by invulnerability (hmm possible not...)
    ATTRIBUTES_HEARTBEAT_RESIST_CHECK				= 0x40000000, // 30 random chance the effect will end TODO: implement core support
    ATTRIBUTES_CANT_CANCEL							= 0x80000000  // 31 positive aura can't be canceled
};

enum AttributesEx
{
	ATTRIBUTESEX_NULL								= 0x00000000, //  2^x
	ATTRIBUTESEX_DISMISS_PET						= 0x00000001, //  0 for spells without this flag client doesn't allow to summon pet if caster has a pet
	ATTRIBUTESEX_DRAIN_ALL_POWER					= 0x00000002, //  1 use all power (Only paladin Lay of Hands and Bunyanize)
	ATTRIBUTESEX_CHANNELED_1						= 0x00000004, //  2 clientside checked? cancelable?
	ATTRIBUTESEX_CANT_BE_REDIRECTED					= 0x00000008, //  3
	ATTRIBUTESEX_IGNORE_IN_FRONT					= 0x00000010, //  4 stealth and whirlwind, ATTRIBUTESEX_UNK4
	ATTRIBUTESEX_NOT_BREAK_STEALTH					= 0x00000020, //  5 Not break stealth
	ATTRIBUTESEX_CHANNELED_2						= 0x00000040, //  6
	ATTRIBUTESEX_CANT_BE_REFLECTED					= 0x00000080, //  7
	ATTRIBUTESEX_CANT_TARGET_IN_COMBAT				= 0x00000100, //  8 can target only out of combat units
	ATTRIBUTESEX_MELEE_COMBAT_START					= 0x00000200, //  9 player starts melee combat after this spell is cast
	ATTRIBUTESEX_NO_THREAT							= 0x00000400, // 10 no generates threat on cast 100% (old NO_INITIAL_AGGRO)
	ATTRIBUTESEX_UNK11								= 0x00000800, // 11 aura
	ATTRIBUTESEX_IS_PICKPOCKET						= 0x00001000, // 12 Pickpocket
	ATTRIBUTESEX_FARSIGHT							= 0x00002000, // 13 Client removes farsight on aura loss
	ATTRIBUTESEX_CHANNEL_TRACK_TARGET				= 0x00004000, // 14 Client automatically forces player to face target when channeling
	ATTRIBUTESEX_DISPEL_AURAS_ON_IMMUNITY			= 0x00008000, // 15 remove auras on immunity
	ATTRIBUTESEX_UNAFFECTED_BY_SCHOOL_IMMUNE		= 0x00010000, // 16 on immunity
	ATTRIBUTESEX_UNAUTOCASTABLE_BY_PET				= 0x00020000, // 17
	ATTRIBUTESEX_UNK18								= 0x00040000, // 18 stun, polymorph, daze, hex
	ATTRIBUTESEX_CANT_TARGET_SELF					= 0x00080000, // 19
	ATTRIBUTESEX_REQ_COMBO_POINTS1					= 0x00100000, // 20 Req combo points on target
	ATTRIBUTESEX_UNK21								= 0x00200000, // 21
	ATTRIBUTESEX_REQ_COMBO_POINTS2					= 0x00400000, // 22 Req combo points on target
	ATTRIBUTESEX_UNK23								= 0x00800000, // 23
	ATTRIBUTESEX_IS_FISHING							= 0x01000000, // 24 only fishing spells
	ATTRIBUTESEX_UNK25								= 0x02000000, // 25
	ATTRIBUTESEX_UNK26								= 0x04000000, // 26 works correctly with [target=focus] and [target=mouseover] macros?
	ATTRIBUTESEX_UNK27								= 0x08000000, // 27 melee spell?
	ATTRIBUTESEX_DONT_DISPLAY_IN_AURA_BAR			= 0x10000000, // 28 client doesn't display these spells in aura bar
	ATTRIBUTESEX_CHANNEL_DISPLAY_SPELL_NAME			= 0x20000000, // 29 spell name is displayed in cast bar instead of 'channeling' text
	ATTRIBUTESEX_ENABLE_AT_DODGE					= 0x40000000, // 30 Overpower
	ATTRIBUTESEX_UNK31								= 0x80000000  // 31
};

enum AttributesExB
{
	ATTRIBUTESEXB_NULL								= 0x00000000, //  2^x
	ATTRIBUTESEXB_CAN_TARGET_DEAD					= 0x00000001, //  0 can target dead unit or corpse
	ATTRIBUTESEXB_UNK1								= 0x00000002, //  1 vanish, shadowform, Ghost Wolf and other
	ATTRIBUTESEXB_CAN_TARGET_NOT_IN_LOS				= 0x00000004, //  2 26368 4.0.1 dbc change
	ATTRIBUTESEXB_UNK3								= 0x00000008, //  3
	ATTRIBUTESEXB_DISPLAY_IN_STANCE_BAR				= 0x00000010, //  4 client displays icon in stance bar when learned, even if not shapeshift
	ATTRIBUTESEXB_AUTOREPEAT_FLAG					= 0x00000020, //  5
	ATTRIBUTESEXB_CANT_TARGET_TAPPED				= 0x00000040, //  6 target must be tapped by caster
	ATTRIBUTESEXB_UNK7								= 0x00000080, //  7
	ATTRIBUTESEXB_UNK8								= 0x00000100, //  8 not set in 3.0.3
	ATTRIBUTESEXB_UNK9								= 0x00000200, //  9
	ATTRIBUTESEXB_UNK10								= 0x00000400, // 10 related to tame
	ATTRIBUTESEXB_HEALTH_FUNNEL						= 0x00000800, // 11
	ATTRIBUTESEXB_UNK12								= 0x00001000, // 12 Cleave, Heart Strike, Maul, Sunder Armor, Swipe
	ATTRIBUTESEXB_PRESERVE_ENCHANT_IN_ARENA			= 0x00002000, // 13 Items enchanted by spells with this flag preserve the enchant to arenas
	ATTRIBUTESEXB_UNK14								= 0x00004000, // 14
	ATTRIBUTESEXB_UNK15								= 0x00008000, // 15 not set in 3.0.3
	ATTRIBUTESEXB_TAME_BEAST						= 0x00010000, // 16
	ATTRIBUTESEXB_NOT_RESET_AUTO_ACTIONS			= 0x00020000, // 17 don't reset timers for melee autoattacks (swings) or ranged autoattacks (autoshoots)
	ATTRIBUTESEXB_REQ_DEAD_PET						= 0x00040000, // 18 Only Revive pet and Heart of the Phoenix
	ATTRIBUTESEXB_NOT_NEED_SHAPESHIFT				= 0x00080000, // 19 does not necessarily need shapeshift
	ATTRIBUTESEXB_UNK20								= 0x00100000, // 20
	ATTRIBUTESEXB_DAMAGE_REDUCED_SHIELD				= 0x00200000, // 21 for ice blocks, pala immunity buffs, priest absorb shields, but used also for other spells -> not sure!
	ATTRIBUTESEXB_UNK22								= 0x00400000, // 22 Ambush, Backstab, Cheap Shot, Death Grip, Garrote, Judgements, Mutilate, Pounce, Ravage, Shiv, Shred
	ATTRIBUTESEXB_IS_ARCANE_CONCENTRATION			= 0x00800000, // 23 Only mage Arcane Concentration have this flag
	ATTRIBUTESEXB_UNK24								= 0x01000000, // 24
	ATTRIBUTESEXB_UNK25								= 0x02000000, // 25
	ATTRIBUTESEXB_UNK26								= 0x04000000, // 26 unaffected by school immunity
	ATTRIBUTESEXB_UNK27								= 0x08000000, // 27
	ATTRIBUTESEXB_UNK38								= 0x10000000, // 28
	ATTRIBUTESEXB_CANT_CRIT							= 0x20000000, // 29 Spell can't crit
	ATTRIBUTESEXB_TRIGGERED_CAN_TRIGGER_PROC		= 0x40000000, // 30 spell can trigger even if triggered
	ATTRIBUTESEXB_FOOD_BUFF							= 0x80000000  // 31 Food or Drink Buff (like Well Fed)
};

enum AttributesExC
{
	ATTRIBUTESEXC_NULL							= 0x00000000, //  2^x
	ATTRIBUTESEXC_UNK0							= 0x00000001, //  0
	ATTRIBUTESEXC_UNK1							= 0x00000002, //  1
	ATTRIBUTESEXC_UNK2							= 0x00000004, //  2
	ATTRIBUTESEXC_BLOCKABLE_SPELL				= 0x00000008, //  3 Only dmg class melee in 3.1.3
	ATTRIBUTESEXC_IGNORE_RESURRECTION_TIMER		= 0x00000010, //  4 you don't have to wait to be resurrected with these spells
	ATTRIBUTESEXC_UNK5							= 0x00000020, //  5
	ATTRIBUTESEXC_UNK6							= 0x00000040, //  6
	ATTRIBUTESEXC_STACK_FOR_DIFF_CASTERS		= 0x00000080, //  7 separate stack for every caster
	ATTRIBUTESEXC_ONLY_TARGET_PLAYERS			= 0x00000100, //  8 can only target players
	ATTRIBUTESEXC_TRIGGERED_CAN_TRIGGER_PROC_2	= 0x00000200, //  9 triggered from effect?
	ATTRIBUTESEXC_MAIN_HAND						= 0x00000400, // 10 Main hand weapon required
	ATTRIBUTESEXC_BATTLEGROUND					= 0x00000800, // 11 Can only be cast in battleground
	ATTRIBUTESEXC_ONLY_TARGET_GHOSTS			= 0x00001000, // 12
	ATTRIBUTESEXC_DONT_DISPLAY_CHANNEL_BAR		= 0x00002000, // 13 Clientside attribute - will not display channeling bar
	ATTRIBUTESEXC_IS_HONORLESS_TARGET			= 0x00004000, // 14 "Honorless Target" only this spells have this flag
	ATTRIBUTESEXC_PLAYER_RANGED_SPELLS         	= 0x00008000, // 15 Auto Shoot, Shoot, Throw,  - this is autoshot flag
	ATTRIBUTESEXC_CANT_TRIGGER_PROC				= 0x00010000, // 16 confirmed with many patchnotes
	ATTRIBUTESEXC_NO_INITIAL_AGGRO				= 0x00020000, // 17 Soothe Animal, 39758, Mind Soothe
	ATTRIBUTESEXC_IGNORE_HIT_RESULT				= 0x00040000, // 18 Spell should always hit its target
	ATTRIBUTESEXC_DISABLE_PROC					= 0x00080000, // 19 during aura proc no spells can trigger (20178, 20375)
	ATTRIBUTESEXC_DEATH_PERSISTENT				= 0x00100000, // 20 Death persistent spells
	ATTRIBUTESEXC_UNK21							= 0x00200000, // 21 unused
	ATTRIBUTESEXC_REQ_WAND						= 0x00400000, // 22 Req wand
	ATTRIBUTESEXC_UNK23							= 0x00800000, // 23
	ATTRIBUTESEXC_REQ_OFFHAND					= 0x01000000, // 24 Req offhand weapon
	ATTRIBUTESEXC_NO_HEALING_BONUS				= 0x02000000, // 25 no cause spell pushback ?
	ATTRIBUTESEXC_CAN_PROC_WITH_TRIGGERED		= 0x04000000, // 26 auras with this attribute can proc from triggered spell casts with ATTRIBUTESEXC_TRIGGERED_CAN_TRIGGER_PROC_2 (67736 + 52999)
	ATTRIBUTESEXC_DRAIN_SOUL					= 0x08000000, // 27 only drain soul has this flag
	ATTRIBUTESEXC_UNK28							= 0x10000000, // 28
	ATTRIBUTESEXC_NO_DONE_BONUS					= 0x20000000, // 29 Ignore caster spellpower and done damage mods?  client doesn't apply spellmods for those spells
	ATTRIBUTESEXC_DONT_DISPLAY_RANGE			= 0x40000000, // 30 client doesn't display range in tooltip for those spells
	ATTRIBUTESEXC_UNK31							= 0x80000000  // 31
};

enum AttributesExD
{
	ATTRIBUTESEXD_NULL						= 0x00000000, //  2^x
	ATTRIBUTESEXD_IGNORE_RESISTANCES		= 0x00000001, //  0 spells with this attribute will completely ignore the target's resistance (these spells can't be resisted)
	ATTRIBUTESEXD_PROC_ONLY_ON_CASTER		= 0x00000002, //  1 proc only on effects with TARGET_UNIT_CASTER?
	ATTRIBUTESEXD_UNK2						= 0x00000004, //  2
	ATTRIBUTESEXD_UNK3						= 0x00000008, //  3
	ATTRIBUTESEXD_UNK4						= 0x00000010, //  4 This will no longer cause guards to attack on use??
	ATTRIBUTESEXD_UNK5						= 0x00000020, //  5
	ATTRIBUTESEXD_NOT_STEALABLE				= 0x00000040, //  6 although such auras might be dispellable, they cannot be stolen
	ATTRIBUTESEXD_TRIGGERED					= 0x00000080, //  7 spells forced to be triggered
	ATTRIBUTESEXD_FIXED_DAMAGE				= 0x00000100, //  8 Ignores resilience and any (except mechanic related) damage or % damage taken auras on target.
	ATTRIBUTESEXD_TRIGGER_ACTIVATE			= 0x00000200, //  9 initially disabled / trigger activate from event (Execute, Riposte, Deep Freeze end other)
	ATTRIBUTESEXD_SPELL_VS_EXTEND_COST		= 0x00000400, // 10 Rogue Shiv have this flag
	ATTRIBUTESEXD_UNK11						= 0x00000800, // 11
	ATTRIBUTESEXD_UNK12						= 0x00001000, // 12
	ATTRIBUTESEXD_UNK13						= 0x00002000, // 13
	ATTRIBUTESEXD_DAMAGE_DOESNT_BREAK_AURAS	= 0x00004000, // 14 doesn't break auras by damage from these spells
	ATTRIBUTESEXD_UNK15						= 0x00008000, // 15
	ATTRIBUTESEXD_NOT_USABLE_IN_ARENA		= 0x00010000, // 16
	ATTRIBUTESEXD_USABLE_IN_ARENA			= 0x00020000, // 17
	ATTRIBUTESEXD_AREA_TARGET_CHAIN			= 0x00040000, // 18 (NYI)hits area targets one after another instead of all at once
	ATTRIBUTESEXD_UNK19						= 0x00080000, // 19 proc delayed, after damage or don't proc on absorb?
	ATTRIBUTESEXD_NOT_CHECK_SELFCAST_POWER	= 0x00100000, // 20 supersedes message "More powerful spell applied" for self casts.
	ATTRIBUTESEXD_UNK21						= 0x00200000, // 21 Pally aura, dk presence, dudu form, warrior stance, shadowform, hunter track
	ATTRIBUTESEXD_UNK22						= 0x00400000, // 22 Seal of Command (42058, 57770) and Gymer's Smash 55426
	ATTRIBUTESEXD_UNK23						= 0x00800000, // 23
	ATTRIBUTESEXD_UNK24						= 0x01000000, // 24 some shoot spell
	ATTRIBUTESEXD_IS_PET_SCALING			= 0x02000000, // 25 pet scaling auras
	ATTRIBUTESEXD_CAST_ONLY_IN_OUTLAND		= 0x04000000, // 26 Can only be used in Outland.
	ATTRIBUTESEXD_UNK27						= 0x08000000, // 27
	ATTRIBUTESEXD_UNK28						= 0x10000000, // 28 Aimed Shot
	ATTRIBUTESEXD_UNK29						= 0x20000000, // 29
	ATTRIBUTESEXD_UNK30						= 0x40000000, // 30
	ATTRIBUTESEXD_UNK31						= 0x80000000  // 31 Polymorph (chicken) 228 and Sonic Boom (38052, 38488)
};

enum AttributesExE
{
	ATTRIBUTESEXE_NULL								= 0x00000000, //  2^x
	ATTRIBUTESEXE_UNK0								= 0x00000001, //  0
	ATTRIBUTESEXD_NO_REAGENT_WHILE_PREP				= 0x00000002, //  1 not need reagents if UNIT_FLAG_PREPARATION
	ATTRIBUTESEXE_UNK2								= 0x00000004, //  2
	ATTRIBUTESEXE_USABLE_WHILE_STUNNED				= 0x00000008, //  3 usable while stunned
	ATTRIBUTESEXE_UNK4								= 0x00000010, //  4
	ATTRIBUTESEXE_SINGLE_TARGET_SPELL				= 0x00000020, //  5 Only one target can be apply at a time
	ATTRIBUTESEXE_UNK6								= 0x00000040, //  6
	ATTRIBUTESEXE_UNK7								= 0x00000080, //  7
	ATTRIBUTESEXE_UNK8								= 0x00000100, //  8
	ATTRIBUTESEXE_START_PERIODIC_AT_APPLY			= 0x00000200, //  9 begin periodic tick at aura apply
	ATTRIBUTESEXE_HIDE_DURATION						= 0x00000400, // 10 do not send duration to client
	ATTRIBUTESEXE_ALLOW_TARGET_OF_TARGET_AS_TARGET	= 0x00000800, // 11 (NYI) uses target's target as target if original target not valid (intervene for example)
	ATTRIBUTESEXE_UNK12								= 0x00001000, // 12 Cleave related?
	ATTRIBUTESEXE_HASTE_AFFECT_DURATION				= 0x00002000, // 13 haste effects decrease duration of this
	ATTRIBUTESEXE_UNK14								= 0x00004000, // 14
	ATTRIBUTESEXE_UNK15								= 0x00008000, // 15 Inflicts on multiple targets?
	ATTRIBUTESEXE_SPECIAL_ITEM_CLASS_CHECK			= 0x00010000, // 16 this allows spells with EquippedItemClass to affect spells from other items if the required item is equipped
	ATTRIBUTESEXE_USABLE_WHILE_FEARED				= 0x00020000, // 17 usable while feared
	ATTRIBUTESEXE_USABLE_WHILE_CONFUSED				= 0x00040000, // 18 usable while confused
	ATTRIBUTESEXE_DONT_TURN_DURING_CAST				= 0x00080000, // 19 Blocks caster's turning when casting (client does not automatically turn caster's model to face UNIT_FIELD_TARGET)
	ATTRIBUTESEXE_UNK20								= 0x00100000, // 20
	ATTRIBUTESEXE_UNK21								= 0x00200000, // 21
	ATTRIBUTESEXE_UNK22								= 0x00400000, // 22
	ATTRIBUTESEXE_UNK23								= 0x00800000, // 23
	ATTRIBUTESEXE_UNK24								= 0x01000000, // 24
	ATTRIBUTESEXE_UNK25								= 0x02000000, // 25
	ATTRIBUTESEXE_UNK26								= 0x04000000, // 26 aoe related - Boulder, Cannon, Corpse Explosion, Fire Nova, Flames, Frost Bomb, Living Bomb, Seed of Corruption, Starfall, Thunder Clap, Volley
	ATTRIBUTESEXE_DONT_SHOW_AURA_IF_SELF_CAST		= 0x08000000, // 27 Auras with this attribute are not visible on units that are the caster
	ATTRIBUTESEXE_DONT_SHOW_AURA_IF_NOT_SELF_CAST	= 0x10000000, // 28 Auras with this attribute are not visible on units that are not the caster
	ATTRIBUTESEXE_UNK29								= 0x20000000, // 29
	ATTRIBUTESEXE_UNK30								= 0x40000000, // 30
	ATTRIBUTESEXE_UNK31								= 0x80000000  // 31 Forces all nearby enemies to focus attacks caster
};

enum AttributesExF
{
	ATTRIBUTESEXF_NULL							= 0x00000000, //  2^x
    ATTRIBUTESEXF_DONT_DISPLAY_COOLDOWN			= 0x00000001, //  0 client doesn't display cooldown in tooltip for these spells
    ATTRIBUTESEXF_ONLY_IN_ARENA					= 0x00000002, //  1 only usable in arena
    ATTRIBUTESEXF_IGNORE_CASTER_AURAS			= 0x00000004, //  2
    ATTRIBUTESEXF_ASSIST_IGNORE_IMMUNE_FLAG		= 0x00000008, //  3 skips checking UNIT_FLAG_IMMUNE_TO_PC and UNIT_FLAG_IMMUNE_TO_NPC flags on assist
    ATTRIBUTESEXF_UNK4							= 0x00000010, //  4
    ATTRIBUTESEXF_UNK5							= 0x00000020, //  5
    ATTRIBUTESEXF_USE_SPELL_CAST_EVENT			= 0x00000040, //  6 Auras with this attribute trigger SPELL_CAST combat log event instead of SPELL_AURA_START (clientside attribute)
    ATTRIBUTESEXF_UNK7							= 0x00000080, //  7
    ATTRIBUTESEXF_CANT_TARGET_CROWD_CONTROLLED	= 0x00000100, //  8
    ATTRIBUTESEXF_UNK9							= 0x00000200, //  9
    ATTRIBUTESEXF_CAN_TARGET_POSSESSED_FRIENDS	= 0x00000400, // 10 NYI!
    ATTRIBUTESEXF_NOT_IN_RAID_INSTANCE			= 0x00000800, // 11 not usable in raid instance
    ATTRIBUTESEXF_CASTABLE_WHILE_ON_VEHICLE		= 0x00001000, // 12 castable while caster is on vehicle
    ATTRIBUTESEXF_CAN_TARGET_INVISIBLE			= 0x00002000, // 13 ignore visibility requirement for spell target (phases, invisibility, etc.)
    ATTRIBUTESEXF_UNK14							= 0x00004000, // 14
    ATTRIBUTESEXF_UNK15							= 0x00008000, // 15 only 54368, 67892
    ATTRIBUTESEXF_UNK16							= 0x00010000, // 16
    ATTRIBUTESEXF_UNK17							= 0x00020000, // 17 Mount spell
    ATTRIBUTESEXF_CAST_BY_CHARMER				= 0x00040000, // 18 client won't allow to cast these spells when unit is not possessed && charmer of caster will be original caster
    ATTRIBUTESEXF_UNK19							= 0x00080000, // 19 only 47488, 50782
    ATTRIBUTESEXF_ONLY_VISIBLE_TO_CASTER		= 0x00100000, // 20 Auras with this attribute are only visible to their caster (or pet's owner)
    ATTRIBUTESEXF_CLIENT_UI_TARGET_EFFECTS		= 0x00200000, // 21 it's only client-side attribute
    ATTRIBUTESEXF_UNK22							= 0x00400000, // 22 only 72054
    ATTRIBUTESEXF_UNK23							= 0x00800000, // 23
    ATTRIBUTESEXF_CAN_TARGET_UNTARGETABLE		= 0x01000000, // 24
    ATTRIBUTESEXF_UNK25							= 0x02000000, // 25 Exorcism, Flash of Light
    ATTRIBUTESEXF_UNK26							= 0x04000000, // 26 related to player castable positive buff
    ATTRIBUTESEXF_UNK27							= 0x08000000, // 27
    ATTRIBUTESEXF_UNK28							= 0x10000000, // 28 Death Grip
    ATTRIBUTESEXF_NO_DONE_PCT_DAMAGE_MODS		= 0x20000000, // 29 ignores done percent damage mods?
    ATTRIBUTESEXF_UNK30							= 0x40000000, // 30
    ATTRIBUTESEXF_IGNORE_CATEGORY_COOLDOWN_MODS	= 0x80000000  // 31 Spells with this attribute skip applying modifiers to category cooldowns
};

enum AttributesExG
{
	ATTRIBUTESEXG_NULL								= 0x00000000, //  2^x
	ATTRIBUTESEXG_UNK0								= 0x00000001, //  0 Shaman's new spells (Call of the ...), Feign Death.
	ATTRIBUTESEXG_IGNORE_DURATION_MODS				= 0x00000002, //  1 Duration is not affected by duration modifiers
	ATTRIBUTESEXG_REACTIVATE_AT_RESURRECT			= 0x00000004, //  2 Paladin's auras and 65607 only.
	ATTRIBUTESEXG_IS_CHEAT_SPELL					= 0x00000008, //  3 Cannot cast if caster doesn't have UnitFlag2 & UNIT_FLAG2_ALLOW_CHEAT_SPELLS
	ATTRIBUTESEXG_UNK4								= 0x00000010, //  4 Only 47883 (Soulstone Resurrection) and test spell.
	ATTRIBUTESEXG_SUMMON_PLAYER_TOTEM				= 0x00000020, //  5 Only Shaman player totems.
	ATTRIBUTESEXG_NO_PUSHBACK_ON_DAMAGE				= 0x00000040, //  6 Does not cause spell pushback on damage
	ATTRIBUTESEXG_UNK7								= 0x00000080, //  7 66218 (Launch) spell.
	ATTRIBUTESEXG_HORDE_ONLY						= 0x00000100, //  8 Teleports, mounts and other spells.
	ATTRIBUTESEXG_ALLIANCE_ONLY						= 0x00000200, //  9 Teleports, mounts and other spells.
	ATTRIBUTESEXG_DISPEL_CHARGES					= 0x00000400, // 10 Dispel and Spellsteal individual charges instead of whole aura.
	ATTRIBUTESEXG_INTERRUPT_ONLY_NONPLAYER			= 0x00000800, // 11 Only non-player casts interrupt, though Feral Charge - Bear has it.
	ATTRIBUTESEXG_UNK12								= 0x00001000, // 12 Not set in 3.2.2a.
	ATTRIBUTESEXG_UNK13								= 0x00002000, // 13 Not set in 3.2.2a.
	ATTRIBUTESEXG_UNK14								= 0x00004000, // 14 Only 52150 (Raise Dead - Pet) spell.
	ATTRIBUTESEXG_UNK15								= 0x00008000, // 15 Exorcism. Usable on players? 100% crit chance on undead and demons?
	ATTRIBUTESEXG_CAN_RESTORE_SECONDARY_POWER		= 0x00010000, // 16 These spells can replenish a powertype, which is not the current powertype.
	ATTRIBUTESEXG_UNK17								= 0x00020000, // 17 Only 27965 (Suicide) spell.
	ATTRIBUTESEXG_HAS_CHARGE_EFFECT					= 0x00040000, // 18 Only spells that have Charge among effects.
	ATTRIBUTESEXG_ZONE_TELEPORT						= 0x00080000, // 19 Teleports to specific zones.
	ATTRIBUTESEXG_UNK20								= 0x00100000, // 20 Blink, Divine Shield, Ice Block
	ATTRIBUTESEXG_UNK21								= 0x00200000, // 21 Not set
	ATTRIBUTESEXG_UNK22								= 0x00400000, // 22
	ATTRIBUTESEXG_UNK23								= 0x00800000, // 23 Motivate, Mutilate, Shattering Throw
	ATTRIBUTESEXG_UNK24								= 0x01000000, // 24 Motivate, Mutilate, Perform Speech, Shattering Throw
	ATTRIBUTESEXG_UNK25								= 0x02000000, // 25
	ATTRIBUTESEXG_UNK26								= 0x04000000, // 26
	ATTRIBUTESEXG_UNK27								= 0x08000000, // 27 Not set
	ATTRIBUTESEXG_CONSOLIDATED_RAID_BUFF			= 0x10000000, // 28 May be collapsed in raid buff frame (clientside attribute)
	ATTRIBUTESEXG_UNK29								= 0x20000000, // 29 only 69028, 71237
	ATTRIBUTESEXG_UNK30								= 0x40000000, // 30 Burning Determination, Divine Sacrifice, Earth Shield, Prayer of Mending
	ATTRIBUTESEXG_CLIENT_INDICATOR					= 0x80000000  // 31
};
