...
void Aura::Remove()
{
	...
	if(caster && caster != m_target && !caster->isAlive())
	{
...
void Aura::SpellAuraModTotalHealthRegenPct(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicHealPct, (float)mod->m_amount, EVENT_AURA_PERIODIC_HEALPERC,
			GetSpellProto()->EffectAmplitude[mod->i], 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
